...
/// <summary>
/// WhatsApp rehberi Excel içe aktarma sihirbazı — CargoImportWindow ile aynı 4 adımlı desen.
/// </summary>
public partial class WhatsAppImportWindow : Window
{
    private readonly IServiceProvider _services;
    private readonly IDialogService _dialogService;
    private readonly WhatsAppImportViewModel _vm;
    ...
    public WhatsAppImportWindow(IServiceProvider services)
    {
        InitializeComponent();
        _services      = services;
        _dialogService = services.GetRequiredService<IDialogService>();
        _vm            = services.GetRequiredService<WhatsAppImportViewModel>();
        DataContext    = _vm;
    }
    ...
    private void TemplateButton_Click(object sender, RoutedEventArgs e)
    {
        var dialog = new SaveFileDialog
        {
            ...
            FileName   = "whatsapp-rehberi-sablonu.xlsx",
            DefaultExt = "xlsx",
        ...
        try
        {
            _services.GetRequiredService<ICargoImportTemplateService>().CreateWhatsAppTemplate(dialog.FileName);
            _dialogService.ShowSuccess($"Şablon kaydedildi.\n{dialog.FileName}");
        }
        catch (Exception ex)
        {
            _ = _services.GetRequiredService<ISystemLogService>()
                .LogErrorAsync("WhatsAppImport", "Şablon dosyası oluşturulamadı", ex, source: nameof(WhatsAppImportWindow));
    ...
    private async void ImportButton_Click(object sender, RoutedEventArgs e)
    {
        if (!_dialogService.ShowConfirmation(
                $"{_vm.SelectedCount} kişi WhatsApp rehberine eklenecek.\n" +
        ...
        var r = result.Data!;
        SummaryMainText.Text = r.ResurrectedCount > 0
            ? $"{r.ImportedCount} kişi rehbere eklendi ({r.ResurrectedCount} silinmiş kayıt geri yüklendi)."
            : $"{r.ImportedCount} kişi rehbere eklendi.";
        SummaryDetailText.Text =
            ...
            "Ayrıntılar Audit Log ekranında (İşlem: WhatsApp Rehberi Toplu İçe Aktarma) görülebilir.";
        ShowStep(4);
    }
